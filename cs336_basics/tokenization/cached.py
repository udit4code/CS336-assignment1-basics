...
class CachedNativeTokenizer(BaseTokenizer):
    ...
    def __init__(self, vocab, merges, special_tokens=None, cache_capacity: int = 65_536):
        ...
        try:
            from ._bpe_native import CachedBPEEngine
        except ImportError as error:
            raise ImportError(
                "CachedNativeTokenizer requires the native extension. Run `uv sync`, then `make native`."
            ) from error

        self._engine = CachedBPEEngine(self.id_to_token, self.merges, cache_capacity)
