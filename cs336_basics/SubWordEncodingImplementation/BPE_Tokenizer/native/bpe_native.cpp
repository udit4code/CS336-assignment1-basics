...
struct V6CandidateGreater {
    bool operator()(const V6Candidate& lhs, const V6Candidate& rhs) const noexcept {
...
class BPEEngineV6 {
  public:
    BPEEngineV6(const py::dict& vocab, const py::list& merges, std::size_t cache_capacity = 65'536)
  ...
  private:
    using CandidateHeap = std::priority_queue<V6Candidate, std::vector<V6Candidate>, V6CandidateGreater>;
    ...
    void push_candidate(CandidateHeap& heap, const std::vector<V6Node>& nodes, std::int32_t left_index) const {
        if (left_index < 0) {
            return;
        }
    ...
    bool candidate_is_valid(const V6Candidate& candidate, const std::vector<V6Node>& nodes) const {
...
PYBIND11_MODULE(_bpe_native, module) {
    ...
    py::class_<BPEEngineV6>(module, "BPEEngineV6")
        .def(
        ...
        .def(
            "encode_pretokens",
            &BPEEngineV6::encode_pretokens,
            py::call_guard<py::gil_scoped_release>()
        )
        .def("cache_stats", &BPEEngineV6::cache_stats)
        .def("cache_size", &BPEEngineV6::cache_size)
        .def("clear_cache", &BPEEngineV6::clear_cache);
}
